...
contract SandwichTest is Test {
    ...
    function setUp() public {
        ...
        vm.prank(attacker); X.approve(address(pool), type(uint256).max);
        vm.prank(attacker); Y.approve(address(pool), type(uint256).max);
        vm.prank(victim);   X.approve(address(pool), type(uint256).max);
    }
